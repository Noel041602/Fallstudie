...
# wenn die Route r zum Zeitpunkt z startet = 1
START_INDIKATOR = {}
for r in ROUTEN:
    start_zeitschritt = int(round(START[r]))
    for z in ZEIT:
        if z == start_zeitschritt:
            START_INDIKATOR[r, z] = 1
        else:
            START_INDIKATOR[r, z] = 0
...
model.setObjective(
    ...
    N_TAGE * quicksum(lkw_fahrt_Route_mit_typ[f, r, "Diesel"] * DISTANZ_GESAMT[r] * VERBRAUCH["Diesel"] * P_DIESEL
...
for f in FUHRPARK:
    for z in ZEIT:
        ...
        # (39) Zwischenparken am Tag: Verbindung zum Ladepunkt ist am Tag (ZEIT_Tag) erlaubt/möglich
        # Hinweis: Das ist im Modell oft durch (11) und (15) bereits limitiert, hier explizit:
        # Falls z in ZEIT_Tag (25-72) liegt, wird hier die Erlaubnis (>=0) formalisiert
        #model.addCons(
         #   quicksum(lkw_ladt[f, z, l] for l in LADEPUNKT) >= 0,
          #  name=f"Tag_Parken_{f}_{z}"
        #)
...
for f in FUHRPARK:
    for z in ZEIT:
        ...
        if z == 0:
            continue
        route_start_fz = quicksum(lkw_fahrt_r[f, r] * START_INDIKATOR[r, z] for r in ROUTEN)
        for l in LADEPUNKT:
            model.addCons(
                lkw_verbunden_ladepunkt[f, z, l] >= lkw_verbunden_ladepunkt[f, z-1, l] - route_start_fz,
...
for f in FUHRPARK:
    if f > 1:
        model.addCons(
            quicksum(lkw_hat_typ[f, t] for t in FTypen) <= 
            quicksum(lkw_hat_typ[f-1, t] for t in FTypen),
...
#model.setRealParam("limits/time", 120) # Gib ihm 5 Minuten Zeit
model.optimize()
...
if model.getNSols() > 0:
    ...
    print(f"Erstelle detailliertes Protokoll in 'Simulationsergebnis_Gesamt.txt'...")
 
    # --- TEIL B: SCHREIBEN ALLER DETAILS IN DIE DATEI ---
    log_filename = "Simulationsergebnis_Gesamt.txt"
    with open(log_filename, "w", encoding="utf-8") as f_out:
        ...
        # 2) GEKAUFTE FLOTTE (Zusammenfassung)
        ...
        diesel_var = N_TAGE * sum(model.getVal(lkw_fahrt_Route_mit_typ[f, r, "Diesel"]) * (DISTANZ_GESAMT[r] * VERBRAUCH["Diesel"] * P_DIESEL) for f in FUHRPARK for r in ROUTEN)
        ...
        strom_var = N_TAGE * sum(model.getVal(momentane_leistung_an_lkw[f, z, l]) * 0.25 * P_STROM for f in FUHRPARK for z in ZEIT for l in LADEPUNKT)
        ...
        # 4) INFRASTRUKTUR & INVESTITIONSBEGRÜNDUNG
        ...
        # 7) VOLLSTÄNDIGER AKTIVITÄTS-LOG
        f_out.write(f"\n[5 - AKTIVITÄTS-LOG]\n")
